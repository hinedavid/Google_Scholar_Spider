...
based on: module to get google search results using Scrapy
...
class GoogleSearch(Spider):
    ...
    def parse(self, response):
        ...
        if self.setting_data['type_of_parse'] == 'google_search':
            ...
            google_search_links_list =  sel.xpath('//h3[@class="gs_rt"]/a/@href').extract()
...
############# Get meta info from website########################################################################################################
     ...
     ############################# Google Format ##########################################################################
            ...
            ##Get doi google Format    
            citation_doi = sel.xpath('/html/head/meta[@name="citation_doi"]/@content').extract()
            if len(citation_doi)>0:
                citation_doi = citation_doi[0].encode(errors='replace') #replace any unknown character with ?
                final_doi = citation_doi
                
            ##Get publisher google Format    
            citation_publisher = sel.xpath('/html/head/meta[@name="citation_publisher"]/@content').extract()
            if len(citation_publisher)>0:
                citation_publisher = citation_publisher[0].encode(errors='replace') #replace any unknown character with ? 
                final_publisher = citation_publisher
                
            ##Get author google Format    
            citation_author = sel.xpath('/html/head/meta[@name="citation_author"]/@content').extract()
            if len(citation_author)>0:
                citation_author = citation_author[0].encode(errors='replace') #replace any unknown character with ? 
                final_author = citation_author
                
            ##Get title google Format    
            citation_title = sel.xpath('/html/head/meta[@name="citation_title"]/@content').extract()
            if len(citation_title)>0:
                citation_title = citation_title[0].encode(errors='replace') #replace any unknown character with ? 
                final_title = citation_title
                
    ########################################### Dublin Core Format ########################################################          
                
            ##Get doi Dublin Core Format    
            DC_identifier = sel.xpath('/html/head/meta[@name="DC.identifier"]/@content').extract()
            if len(DC_identifier)>0:
                DC_identifier = DC_identifier[0].encode(errors='replace') #replace any unknown character with ?
                final_doi = DC_identifier
                
            ##Get publisher Dublin Core Format      
            DC_Publisher = sel.xpath('/html/head/meta[@name="DC.Publisher"]/@content').extract()
            if len(DC_Publisher)>0:
                DC_Publisher = DC_Publisher[0].encode(errors='replace') #replace any unknown character with ? 
                final_publisher = DC_Publisher
                
            ##Get author Dublin Core Format     
            DC_Contributor = sel.xpath('/html/head/meta[@name="DC.Contributor"]/@content').extract()
            if len(DC_Contributor)>0:
                DC_Contributor = DC_Contributor[0].encode(errors='replace') #replace any unknown character with ? 
                final_author = DC_Contributor
                
            ##Get title Dublin Core Format     
            DC_Title = sel.xpath('/html/head/meta[@name="DC.Title"]/@content').extract()
            if len(DC_Title)>0:
                DC_Title = DC_Title[0].encode(errors='replace') #replace any unknown character with ? 
                final_title = DC_Title
                
############# END Get meta info from website##########################################################################################################
            ...
            with open(RESULT_FILE,'a') as f:
                ...
                for n in range(4): f.write('\n')
                f.write("url:" + response.url + '\n')
                f.write("Author:"+ str(final_author) + '\n')
                f.write("doi:"+ str(final_doi) + '\n')
                f.write("Publisher:"+ str(final_publisher) + '\n')
